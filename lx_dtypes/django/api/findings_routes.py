...
from typing import TYPE_CHECKING, Any, Callable, Dict, List, NoReturn, Optional, Protocol, Set, TypeVar, cast
...
def register_findings_routes(
    ...
    *,
    ...
    def choices_by_classification(
        ...
        if not classification:
            api_error(404, "not-found", f"Classification '{classification_id}' not found.")
        assert classification is not None
    ...
    def patch_patient_finding(
        ...
        module_name = _findings_module_name()
        patient_finding = (
            _active_patient_findings_queryset(orm_models).filter(id=patient_finding_id).first()
        )
