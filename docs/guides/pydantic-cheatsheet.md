...
This guide consolidates strict typing, date handling, serialization, and
validation into reusable patterns for `lx_dtypes` and its consumers.

## The Pydantic V2 Lifecycle

Understanding when validation and serialization run is essential when choosing
the patterns below.

## 1. Shared Base Model

Use a project base model when schemas share strict input hygiene and model
configuration. Consumers should prefer an existing package-owned base model
over introducing another one.
...
class AppBaseModel(BaseModel):
    model_config = ConfigDict(
        ...
        extra="forbid",  # Reject undeclared fields at the boundary.
        validate_default=True,  # Validate defaults as well as supplied values.
        populate_by_name=True,  # Accept a field name when an alias is defined.
    )
```
...
class TimestampModel(AppBaseModel):
    # Enforce timezone information in input.
    event_time: AwareDatetime

    # Calculate the default at runtime and store it as UTC.
    created_at: AwareDatetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )

    # Wrong: a static default is calculated at import time.
...
Use computed fields for values that should appear in serialized output but
should not be persisted directly.
...
class FileInput(BaseModel):
    ...
    # Use specialized validators for paths that must already exist.
    template_path: FilePath
    output_dir: DirectoryPath

    # Accept strings, but normalize them to absolute paths.
...
class LazyPathModel(BaseModel):
    # Use plain Path when the resource may not exist yet.
